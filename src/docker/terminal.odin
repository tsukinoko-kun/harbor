...
@(private)
launch_terminal_with_docker_exec :: proc(container_id: string, container_name: string, shell: string) {
	when ODIN_OS == .Darwin {
...
launch_terminal_linux :: proc(container_id: string, container_name: string, shell: string) {
	...
	for term_cmd in terminal_commands {
		...
		check_cmd := strings.concatenate({"which ", term_name, " >/dev/null 2>&1"}, context.temp_allocator)
	...
	fmt.eprintf("No terminal emulator found. Please install one of: gnome-terminal, konsole, xfce4-terminal, xterm\n")
}
